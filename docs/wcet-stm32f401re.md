...
2. **Hardware-isolated interrupt stack.** Confirmed in `docs/wcet.md` §4.2 and
   ...
   portable constant on this arch, unlike Xtensa, where the ESP32-S3 fix
   needed a 4x, empirically discovered stack increase because the entire
   handler runs on the interrupted task's own stack.
3. **Architecturally guaranteed clock**, not an assumption. `rivet-bsp-
   ...
   independently re-measured here, but it does not need to be: it is the
   chip's guaranteed power-on behavior with zero configuration code in
   between, unlike ESP32-S3's assumed-but-unconfirmed 40 MHz.
...
single-core target: the true worst case is ARM's fixed entry latency plus the
...
real hardware measurement can produce: not "usually 86, sometimes more," but
measured identically 500/500 times. 86 cycles = 5.375 µs; 391 cycles = 24.4 µs, both at
16 MHz.

---

## 3. Critical-section WCET: normal operation versus terminal-only
...
`rivet-bsp-stm32f401re`'s `__rivet_board_init` calls
`rivet::console::enable_irq_tx()`. Every normal `console::write_str` call goes
through `write_bytes_irq`: push bytes into a 256-byte SPSC ring under
`critical::enter` (O(1) per byte, no UART wait in the common case; the ring
absorbs it, actual transmission happens later, asynchronously, off the
critical path), then return. **Confirmed by direct source read**
(`rivet-bsp-stm32f401re/src/lib.rs`'s `__rivet_board_console_write` is a bare
polling loop, *not* wrapped in `critical::enter` at the board level at all)
**and by measurement**: the bulk of every `critsec` histogram captured
(more than 99.9% of samples in every workload) sits in the 32-1023 cycle
range (2-64 µs), consistent with O(1)/O(MAX_PTASKS)-bounded kernel
bookkeeping, not UART wait.
...
**Measured**: `flush_sync`'s actual cost across all workloads run,
...
(8N1, 10 bits/byte, so 86.8 µs/byte): **256 × 86.8 µs is roughly 22.2 ms,
roughly 355,600 cycles.** This is the true upper bound to cite for
`flush_sync`/system-termination latency, even though nothing close to it
was ever observed.
...
wcet.md` §7.1 (at most 87 bounded operations for a full mutex unlock)
applies as the *mechanism's* bound; 8191 cycles is what that mechanism
actually cost, once, under real contention, including memory access latency
...
`priority_inversion_bench` (purpose-built, run directly on hardware)
...
```
high_wait_cycles=40548          (roughly 2.53 ms at 16 MHz)
low_critical_section_cycles=56443   (roughly 3.53 ms at 16 MHz)
...
DERIVED): lock at most 22 bounded operations, unlock at most 87 bounded
operations. Unchanged on this arch, and consistent with §3.3's 8191-cycle
normal-operation ceiling once real memory/pipeline cost per operation is
included.
...
| Scheduling-decision algorithmic bound | O(1) + one at most 16-iteration bounded scan | DERIVED, arch-independent (`docs/wcet.md` §5). Unchanged, and the measured 984-1048 cycle figure is consistent with this bound plus real memory-access cost, not evidence of an unbounded path |

**This table alone is the clearest evidence for §1's claim.** The same
algorithmic scheduler, on a single-core target, shows a tight roughly 2x
measured spread (512-1048 cycles) with *no* outliers reaching into the tens
of thousands, where the identical algorithm on dual-core Xtensa showed a
...
| Observed real-task stack watermarks | 64-512 bytes used, out of 256-4096-byte allocations | MEASURED, `rivet::report()` output across all four workloads |

No stack-related crash, corruption, or watermark violation occurred in any of
the four workloads run on real hardware. Given §1.2's structural
...
| Bound | Value | Status |
|---|---|---|
| Interrupt latency (tick, normal operation) | at most 6666 cycles (416.6 µs) | MEASURED worst case, 4 workloads; architectural floor 12 cycles |
| Nested interrupt latency | 86 cycles (5.375 µs), **zero variance, 500/500 samples** | MEASURED |
| Context-switch mechanism cost | at most 24 cycles HW + 10 insns SW | ARCHITECTURAL + DERIVED |
| Scheduling-decision cost | at most 1048 cycles (65.5 µs) | MEASURED worst case, 4 workloads; at most 16-iteration bound DERIVED |
| Critical-section hold time, **normal operation** | at most 8191 cycles (512 µs) | MEASURED worst case, 4 workloads |
| Mutex lock kernel overhead | at most 22 bounded operations | DERIVED, exact |
| Mutex unlock kernel overhead | at most 87 bounded operations | DERIVED, exact |
...
2. **Does not cover**: the interval from a fault/watchdog-timeout/`exit()`
   ...
   256 queued console bytes (§3.2), bounded at roughly 22.2 ms (DERIVED
   worst case; 42,308-42,486 cycles, roughly 2.64-2.66 ms MEASURED typical,
   across four independent workloads), by design, since this is the
   mechanism that guarantees a fault's diagnostic message is not lost. **A
   hard-real-time task set using this kernel on this board must treat
   "system has faulted" as "no further deadlines are being met," which is
   standard practice** (DO-178C-style systems universally treat the
   transition into a failure-response state as outside the nominal timing
   budget), not a gap unique to Rivet.
3. **Does not cover**: application-level critical-section bodies. The kernel
   bounds its own mutex mechanism exactly (at most 87 operations); it cannot
   and does not bound what an application does *between* `lock()` and
   `drop()`. `docs/wcet.md` §7.2's warning applies here too: an
   application-level critical section that itself calls `console::write_str`
   with output long enough to overflow the 256-byte ring reintroduces
   §3.2's baud-rate-bound cost into normal operation. **This declaration
   assumes application code does not do that**, an auditable,
   one-line-per-call-site property, not an unverifiable one.
4. **Does not cover**: the HSI clock's absolute accuracy (±1% per ST's own
   datasheet, not re-measured against an external reference here), relevant
   only if this system needs to correlate timing against an external wall
   clock; irrelevant to the *relative* cycle-count bounds in this document,
   which hold regardless of the oscillator's absolute trim.
