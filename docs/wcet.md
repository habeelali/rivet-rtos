...
- **ASSUMED** — a configuration value taken as given (e.g. clock rate) that this
  session did not independently verify against real silicon.
...
| QEMU RISC-V virt | N/A | No real silicon in this environment. QEMU's TCG backend does not model cycle-accurate timing. Figures for this target are **instruction counts** (exact, from the real assembly) with a **derived** cycle estimate assuming a simple in-order RV32IMAC reference core (comparable to a SiFive E-class core) at 1 cycle/instruction baseline, +2–3 cycles per taken jump/branch. |
...
## 3. Interrupt latency (assertion → first instruction of the scheduler's own code)
...
| **RISC-V (QEMU virt)** | `mtvec` dispatch → `rivet_trap_handler_rust` called | **34 instructions** exact (save path of `rivet_trap_entry`) + 1 jump (vectored) or fixed offset (direct mode) to reach the handler | **DERIVED** — counted directly from `rivet-arch-riscv/src/lib.rs`'s `global_asm!` (lines 541–575); ≈36–40 cycles at 1 CPI + jump overhead for a reference in-order core |
...
|---|---|---|---|---|
| RISC-V | 28 `sw` + 6 setup = 34 | 4 setup + 28 `lw` + 2 = 34 | **76 instructions**, exact | ≈80–95 cycles, DERIVED (1 CPI + `mret`/`call` overhead) |
| Cortex-M3 | 5 sw insns (r4–r11 `stmia`, 8 regs) + 12-cycle HW auto-stack (r0–r3/r12/LR/PC/xPSR) | 5 sw insns (`ldmia`) + 12-cycle HW auto-unstack | **10 sw insns + 24 HW cycles** | ≈54 cycles, DERIVED (5 asm cost per ARM per-instruction table: `stmia`/`ldmia` of 8 registers ≈ N+1 cycles each on M3) + 24 ARCHITECTURAL |
| Xtensa (ESP32-S3) | `xtensa-lx-rt`'s `SAVE_CONTEXT`+`SPILL_REGISTERS` (full window flush) + this crate's `CONTEXTS[tid]` copy (136-byte `memcpy`) | mirror, `RESTORE_CONTEXT` + `CONTEXTS[resume]` copy | — | **MEASURED** via `Kind::DispatchDecision`: dominant bucket 512–1023 cycles (uncontended, single-hart or lock-uncontested case); **worst-observed 97,133–166,597 cycles** under real dual-hart contention (see §6 — this is `critical::enter` lock-wait time, not switch mechanism cost) |
...
stack. This is not hypothetical: it is exactly what forced this session's
dual-hart race fix (`docs/realtime.md` §15) to need a 4x task-stack increase
...
| Dispatch commit (`set_state`, `set_current`, `on_dispatch`) | O(1) + O(MAX_HARTS) | `ready_add`/`ready_remove` are O(1) bitmap ops; `wake_other_harts` loops at most `MAX_HARTS - 1` times (2 on this session's dual-hart ESP32-S3 builds) |

**Total worst case: a small, fixed number of O(1) operations plus one 16-iteration
bounded scan** — genuinely O(MAX_PTASKS), not larger. This is the mechanism's
...
assumes is short. Two independent findings from this session's real-hardware
measurement say it is not always short, for reasons that have nothing to do with
the scheduler's own algorithm.
...
Once the ~128-byte hardware FIFO saturates (any string longer than the FIFO's
headroom, printed faster than the wire drains — true by a huge margin at 40 MHz
CPU clock vs. serial baud rate), **the loop is waiting on physical transmission
time, not CPU cycles** — at an assumed 115200 baud (8N1, 10 bits/byte), that's
**≈86.8 µs, ≈3472 cycles at 40 MHz, per byte**, for the entire remainder of the
string, with interrupts (and the cross-hart lock) held the whole time.

**MEASURED**, `Kind::CriticalSection` max, exact values via this session's added
max-cycle tracking (not just the log2-bucketed histogram, whose top bucket
`[2^15, ∞)` cannot itself answer "how large," only "at least 32768"):

| Workload | Config | `critsec` max (cycles) | ≈ time @ 40 MHz |
|---|---|---|---|
...
console output length, consistent with the ≈63-byte-at-115200-baud arithmetic
above. **This is a real, reproducible, ~5.5 ms critical section — roughly three
orders of magnitude larger than the scheduler's own algorithmic bound in §5.**
...
string length past ~128 bytes) and already reaches multiple milliseconds with
ordinary diagnostic output. **Any code path that can reach this — including
fault/panic handlers — must be excluded from a hard-real-time task's blocking
analysis, or the console must move to the already-implemented interrupt-driven
path (`write_bytes_irq`, SPSC-buffered, bounded by ring size not string length)
before this board can carry a genuine hard-real-time guarantee.**
...
happen to run (which §6.1 shows can itself be large). This session measured
`Kind::DispatchDecision` — a timestamp that spans `critical::enter`'s own acquire
wait plus the actual decision — reaching **166,597 cycles** worst-observed on
real dual-hart hardware, consistent with contending against a §6.1-class
console write elsewhere in the system.

**This is a genuine limitation of the current lock, not a measurement artifact**:
a formal hard-RTOS certification cannot derive a closed-form bound on
`critical::enter` contention from the algorithm alone. The practical bound is
"the longest critical section anywhere in the linked binary" (§6.1's ~5.5 ms,
today), which is a property of the *application*, not a kernel-provided
guarantee.
...
|---|---|---|
| `lock()` (uncontended) | 1 CAS (`try_acquire`) + `push_held` (≤4-slot linear scan) + `boost_holder` (O(1)) | **≤ 6 bounded ops** |
| `lock()` (contended, blocks) | above + `add_waiter` (≤16-slot CAS scan) | **≤ 22 bounded ops** |
| `PriorityMutexGuard::drop` (unlock) | owner swap (O(1)) + `remove_held` (≤4) + effective-priority recompute (≤4 held mutexes × ≤16-slot `highest_waiter_priority` scan **each** = ≤64) + `set_effective_priority` (O(1)) + `wake_all_waiters` (≤16 waiters × O(1) each: `unblock` + `cancel_ptask_deadline`) | **≤ 87 bounded ops** — the single most expensive *bounded* kernel path in the crate |
...
configuration, just with a real constant (~87) worth stating exactly rather than
waving away as "small."
...
where **n** = number of lower-priority tasks in the system that can lock a mutex
the blocked task also needs, **m** = number of distinct mutexes the blocked task
can wait on (bounded by how many the *lower-priority* holder(s) can nest, ≤
`MAX_HELD_MUTEXES = 4`), **C_max** = the longest *application* critical-section
body protected by any such mutex, and **O_unlock** ≈ 87 bounded operations
(§7.1) is the kernel's own fixed unlock overhead added on top.

**The kernel can bound `O_unlock` exactly. It cannot bound `C_max`** — that is
inherently an application property. §6.1 is the concrete warning: if any
...
|---|---|---|---|
| RISC-V | **128 bytes**, exact (`FRAME_WORDS=32 × 4`), charged once regardless of handler complexity — handler body runs on a separate 2 KB per-hart `.isr_stack` | None (isolated) | — |
...
## 9. Known limitations found during this analysis (report, not fixed, per scope)
...
1. **`crate::latency::Kind::SchedulingWake`'s cycle-delta computation is not
   ...
   (≈107 s at the assumed 40 MHz), a sample straddling the wrap produces a
   bogus multi-billion-cycle "latency." Observed directly this session: max
   reported as 4,282,267,520–4,282,837,995 cycles (≈107 s — the wraparound
   period itself, not a real latency) on longer-running workloads, while a
...
3. **`console_write`'s critical section is baud-rate-, not CPU-, bound**
   (§6.1) — the most severe finding in this document. Not something this
   session fixed (out of scope for an analysis pass), but it should be
   treated as blocking for any actual hard-real-time certification of the
   ESP32-S3 board as shipped.
4. **ESP32-S3's `CPU_HZ = 40_000_000` is unverified against real silicon**
   ...
   uncertainty; the cycle-domain figures (this session's actual measurements)
   do not.
...
| Worst measured critical section | not measured (no real HW) | not measured (no real HW) | **219,288–222,506 cycles ≈ 5.5 ms, MEASURED** (console I/O, §6.1) |
| Mutex unlock, kernel overhead | ≤87 bounded ops, DERIVED (arch-independent) | same | same |
...
(§5, §7.1) — every kernel-internal operation this session traced terminates in a
