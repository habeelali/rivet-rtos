...
- Two boards sharing the same CPU architecture (e.g. two different
  Cortex-M3 boards) couldn't coexist in one workspace, since the arch
  selection was keyed on the *target triple*, not the board.
...
the fault dump and by `rivet::report()`. It works identically on both
...
docs/realtime.md        real-time characterization log: every timing bug
                        found and fixed on real hardware, including the
                        ESP32-S3 dual-hart race (§15)
...
docs/rpi3b-guarantees.md
                        which of the Pi 3B figures may be designed
                        against: the architectural guarantees, a scoped
                        declaration with a stated engineering margin, and
                        the shared-L2 limit that stops it going further
...
`plan.md` (a local development log kept throughout this project's
development) is a working document, not part of the tracked repository —
...
cargo xtask soak --target riscv --sim-hours 4   # bounded soak-invariant proof
...
(`main.rs` in each `examples/*` package) walks through three parts in
...
- **`proptest`**: model-based testing: a naive reference scheduler and
...
- **`cargo-llvm-cov`**, wired into CI as an informational (non-gating)
  report. There is no enforced coverage floor yet (see §18).
...
- **`rivet::console::write_str` on the ESP32-S3 board holds `critical::
  ...
  once it exceeds the hardware FIFO's roughly 128-byte headroom**: bound by
  baud rate, not CPU speed, measured at roughly 5.5 ms for ordinary
  `report()` output (`docs/wcet.md` §6.1). Not present as a
  normal-operation hazard on STM32F401RE, whose console is
  interrupt-driven/ring-buffered during normal operation; the equivalent
  unbounded-by-length path there (`flush_sync`) is architecturally
  confined to system termination, not reachable from a running task
  (`docs/wcet-stm32f401re.md` §3).
...
The single source of truth for exactly what's done and what's deferred is
`plan.md`, a local, untracked running log kept current as work lands (see
`CONTRIBUTING.md` for why it isn't part of a fresh clone).
