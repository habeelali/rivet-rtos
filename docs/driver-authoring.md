...
Read this if you're adding a new interrupt-driven peripheral driver to a
`rivet-bsp-*` crate, not designing one from scratch. Every rule here was
found the hard way, on real hardware, building the drivers already in this
tree (`pl022`, `stellaris_i2c`, the STM32F401 I2C and EXTI drivers).
Cross-reference their source for worked examples, not just this doc.
...
does: try_take, register the current task as waiter, try_take again, then
...
run (async fn bodies don't execute until first polled, so the trigger call
...
`rivet::irq::register(irq_num: u32, handler: fn())` takes plain function
...
since the ISR unconditionally disarms them on every entry, so nothing else
will. This was the second real bug found building that driver:
`send_address_async` inherited the enables `start_async` had armed, but
the ISR had already cleared them after the first interrupt, so the second
phase's completion interrupt never fired.
...
- **QEMU's PL022 model**: `RXIM` only asserts once the RX FIFO holds ≥ 4
  bytes, so a transfer shorter than that won't raise the interrupt under
...
- **QEMU's `stellaris-i2c` model**: address-NAK sets `MCS.ERROR` but never
  raises an interrupt, so check it synchronously right after issuing `RUN`,
...
- **STM32's legacy I2C peripheral**: `ADDR` must be cleared by reading
  ...
  needs `BTF`, not just `TXE`, since `TXE` alone can issue `STOP` while the
