...
fn holder(_: &'static Unit) -> ! {
    ...
    let _ = rivet::spawn_ptask!(stack = 1024, priority = 6, entry = waiter_b, arg = UNIT);
    let _ = rivet::spawn_ptask!(stack = 1024, priority = 8, entry = waiter_a, arg = UNIT);
...
fn timeout_task(_: &'static Unit) -> ! {
    ...
    let _ = rivet::spawn_ptask!(stack = 1024, priority = 5, entry = t_holder, arg = UNIT);
    rivet::yield_now();
...
fn main() -> ! {
    ...
    let _ = rivet::spawn_ptask!(stack = 1024, priority = 4, entry = timeout_task, arg = UNIT);
    let _ = rivet::spawn_ptask!(stack = 1024, priority = 3, entry = stress_task, arg = UNIT);
    let _ = rivet::spawn_ptask!(stack = 1024, priority = 3, entry = stress_task, arg = UNIT);
    let _ = rivet::spawn_ptask!(stack = 1024, priority = 2, entry = holder, arg = UNIT);
