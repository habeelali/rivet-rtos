...
fn low_task(_: &'static Unit) -> ! {
    ...
    for _ in 0..N_MEDIUM {
        let _ = rivet::spawn_ptask!(stack = 512, priority = 4, entry = medium_task, arg = UNIT);
    }
    let _ = rivet::spawn_ptask!(stack = 1024, priority = 9, entry = high_task, arg = UNIT);
...
fn main() -> ! {
    ...
    let _ = rivet::spawn_ptask!(stack = 1024, priority = 1, entry = low_task, arg = UNIT);
