...
fn spawner(_: &'static ()) -> ! {
    ...
    for _ in 0..slots_left {
        let r = rivet::spawn_ptask!(stack = 1024, priority = 2, entry = worker, arg = ());
        if r.is_err() {
    ...
    match rivet::spawn_ptask!(stack = 1024, priority = 2, entry = worker, arg = ()) {
...
#[rivet::main]
fn main() -> ! {
    rivet::console::write_str("Rivet stress_max_ptasks\n");

    let _ = rivet::spawn_ptask!(stack = 1024, priority = 1, entry = spawner, arg = ());
