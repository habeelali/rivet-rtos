...
fn supervisor(_: &'static ()) -> ! {
    ...
    let h2 = match rivet::spawn_ptask!(stack = 1024, priority = 2, entry = worker7, arg = ()) {
        Ok(h) => h,
...
fn main() -> ! {
    ...
    let h1 = match rivet::spawn_ptask!(stack = 1024, priority = 2, entry = worker42, arg = ()) {
        Ok(h) => h,
    ...
    let hc = match rivet::spawn_ptask!(stack = 1024, priority = 1, entry = chatter, arg = ()) {
        Ok(h) => h,
    ...
    let _ = rivet::spawn_ptask!(stack = 1024, priority = 3, entry = supervisor, arg = ());
